...
float mix4(float a, float b, float c, float d, float t)
{
    t = fract(t / 4) * 4;
    float acc = mix(a, b, saturate(t));
...
vec2 mix4(vec2 a, vec2 b, vec2 c, vec2 d, float t)
{
    t = fract(t / 4) * 4;
    vec2 acc = mix(a, b, saturate(t));
    acc = mix(acc, c, saturate(t - 1));
    acc = mix(acc, d, saturate(t - 2));
    acc = mix(acc, a, saturate(t - 3));
    return acc;
...
const mat4 bayer_matrix =
    (mat4(0, 8, 2, 10, 12, 4, 14, 6, 3, 11, 1, 9, 15, 7, 13, 5) + 0.5) / 16;

float dither(vec2 uv)
{
    ivec2 xy = ivec2(uv * resolution) & 3;
    return bayer_matrix[xy.x][xy.y] - 0.5;
}
...
vec3 render(vec2 uv)
{
    float amp = pow(spectrum.x, 4);
    float c1 = step(abs(uv.x - 0.5), amp * 0.2);
    
    vec2 disp = vec2(cnoise(vec3(uv * vec2(8, 5), time * 1.48)),
                     cnoise(vec3(uv * 12, time * 1.24)));
    disp *= vec2(0.042, 0.11) * mix(0.2, 1, spectrum.x);
    
    float r = c1 + feedback(disp).r * 0.96;
    float g = (sin(r * 4.2 + time * 2.7) + 1) / 2 * r;
    float b = (sin(r * 2.2 + time * 1.3) + 1) / 2 * r;
    return vec3(r, g, b);
}
